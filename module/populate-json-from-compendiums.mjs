...
export async function populateJSONFromCompendiums() {
  ...
  for (const packInfo of packs) {
    try {
      // Find world compendium by label
      const labelMap = {
        "weapons": "Weapons",
        "armour": "Armour",
        "species": "Species",
      ...
      const pack = game.packs.find(p => p.metadata.label === label && p.metadata.package === "world");
      
      if (!pack) {
        console.warn(`RQ3 | World compendium with label '${label}' not found, skipping export.`);
        continue;
      }
